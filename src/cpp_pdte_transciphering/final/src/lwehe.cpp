...
double SchemeLWE::noise(const Ctxt_LWE& ct, int m) const
{
    int noise = ct.b - parLWE.delta_base * m;
    for (int i = 0; i < parLWE.n; i++)
    {
        noise += ct.a[i] * sk_base[i];
    }
    noise = parLWE.mod_q_base(noise);
    if (0 == noise)
        return 0;
    return log(noise) / log(2);
}
